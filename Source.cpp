...
bool GameOverCheck(int, PLAYER player[3]);
//int main()
//{
	//playerGrid1.display();
	//waitKey(0);
...
//Regular shot
		...
		if (regularShot == true) {
			...
			cout << player[enemyPlayer].grid[x][y] << endl;
		...
		int aWin = GameOverCheck(enemyPlayer, player);
		if (aWin != 0) {
			guessingStage = false;
			break;
		}
...
bool GameOverCheck(int enemyPLAYER, PLAYER player[3])
{
	bool winner = true;
